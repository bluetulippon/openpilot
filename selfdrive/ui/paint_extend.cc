...
void ui_draw_left_hud_infobox1(UIState *s) {
  ...
  if ((*s->sm).updated("carState")) {
    ...
    if((*s->sm)["carState"].getCarState().getSteeringPressed()) {
    ...
    } else if((*s->sm)["carControl"].getCarControl().getAvailableFulltimeLka()) {
      ui_draw_hud_infobox(s, sidebar_fit_x, hud_left_1_y, hud_left_1_w, hud_left_1_h,
...
void ui_draw_left_hud_infobox2(UIState *s) {
  ...
  if ((*s->sm).updated("carState")) {
    if((*s->sm)["carState"].getCarState().getBrake()>0) {
        snprintf(value, sizeof(value), "%.2f°", (*s->sm)["carState"].getCarState().getBrake());
      ...
      if((*s->sm)["carState"].getCarState().getGas()>0) {
        snprintf(value, sizeof(value), "%.2f°",((*s->sm)["carState"].getCarState().getGas()));
        if((*s->sm)["carState"].getCarState().getGasPressed()) {
            ui_draw_hud_infobox(s, sidebar_fit_x, hud_left_2_y, hud_left_2_w, hud_left_2_h,
